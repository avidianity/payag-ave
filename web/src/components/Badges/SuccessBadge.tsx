...
const SuccessBadge = forwardRef<HTMLSpanElement, Props>(({ children, className, ...props }, ref) => {
	return (
		<span ref={ref} className={`w-16 text-center text-xs py-1 px-2 rounded-lg bg-green-500 text-white ${className || ''}`} {...props}>
			{children}
		</span>
	);
});
